...
module GraphMatching
  ...
  class Graph < RGL::AdjacencyGraph
    ...
    def e(m)
      ...
      # numbering the vertices 1 to V and the edges V -4- 1 to
      ...
      # assign a start label and begin a new search) set LABEL(u)
      # ~-- FIRST(u) ~- O.
      #
      # E2 [Choose an edge ] Choose an edge xy, where x ]s an outer
      # vertex. (An edge vw may be chosen twice in a search--once
      # with x = v, and once with x = w.) If no such edge exists,
      # go to E7. (Edges xy can be chosen m an arbitrary order. A
      # posmble chome method m "breadth-first": an outer vertex
      # x = xt is chosen, and edges x~y are chosen m succeeding
      # executions of E2, when all such edges have been chosen, the
      # vertex x~ that was labeled immediately after x~ is chosen,
      # and the process is re-peated for x = x~. This breadth-first
      # method requires that Algorithm E maintain a list of outer
      # vertices, xL, x~ ••• .)
      #
      # E3. [Augment the matching.l If y is unmatched and y ¢ u, set
      # MATE(y) ~--x, call R(x, y): then go to E7 (R completes the
      # augment along path (y)*P(x))
      #
      # E4. [Assign edge labels.] If y is outer, call L, then go to
      # E2 (L assigns edge label n(xy) to nonouter vertmes in P(x)
      # and P(y))
      #
